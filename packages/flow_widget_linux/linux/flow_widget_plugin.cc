...
std::filesystem::path ConfigDirectory() {
  ...
  if (xdg_config != nullptr && xdg_config[0] != '\0') {
    return std::filesystem::path(xdg_config) / "flow_widget";
  }
  const char* home = std::getenv("HOME");
  if (home != nullptr && home[0] != '\0') {
    return std::filesystem::path(home) / ".config" / "flow_widget";
  }
  return std::filesystem::temp_directory_path() / "flow_widget";
}
...
void FlowWidgetPlugin::HandleMethodCall(
  ...
  if (method == "initialize") {
    storage_ = std::make_unique<FlowWidgetStorage>(ConfigDirectory());
    storage_->Load();
    result->Success();
    return;
  }
  ...
  if (method == "getData") {
    ...
    auto wire = storage_->GetData(ReadString(*args, "key"));
    if (!wire.has_value()) {
      result->Success();
      return;
    }
  ...
  if (method == "startLiveActivity" || method == "updateLiveActivity" ||
    ...
    result->Error("unsupported", "Live Activities are not available on Linux.");
    return;
  }
